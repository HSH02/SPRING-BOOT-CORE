...
**D**ependency **I**njection, 일명 DI로 불리는 이 녀석은 Spring 내내 자꾸 이름을 비추면서 드러냅니다.

DI란 무엇일까요? 대체 왜 자꾸 등장하는 것일까요?

간단히 말해, **의존성 주입은 객체의 생성과 의존 관계를 프레임워크가 대신 관리해주는 방식**입니다.

과거 자바 개발에서는 객체를 직접 생성하고 관리해야 했지만, DI를 통해 이러한 과정을 Spring이 대신 처리해줍니다.
...
Apple 객체를 만드는 간단한 코드입니다. 

그러나... 애플리케이션이 커지고 Apple 객체를 여러 군데에서 생성하게 된다면?

객체 생성 및 관리는 어떻게 해야 할까요?

Spring은 **Bean**이라는 개념을 도입했습니다. 

Bean은 **Spring에 의해 관리되는 자바 객체**를 의미합니다.
...
FruitBasket 클래스는 Apple 객체에 의존하고 있습니다. 

`@Autowired` 어노테이션을 통해 Spring은 Apple Bean을 자동으로 주입하고 있습니다. 

이렇게 하면 객체 생성과 관리가 Spring에 의해 자동화되며 개발자는 비즈니스 로직에만 집중할 수 있게 됩니다.

비유를 들자면, 전통적인 자바 개발은 냉장고에서 일일이 재료를 가져와 요리에 넣는 것과 같습니다. 

반면, DI를 사용하면 이미 준비된 재료를 손쉽게 요리에 넣는 것과 비슷합니다.


- 의존성 주입을 사용하는 이유

의존성 주입을 사용하는 주 이유는 **객체 간의 결합도를 낮추고, 코드의 재사용성과 유지보수성을 높이기 위함**입니다.

1. 결합도 감소:
    - 클래스들이 다른 클래스에 의존하지 않고, 인터페이스나 추상화에 의존하게 되어 변경에 유연해집니다. 
2. 재사용성 향상:
   - 의존성이 외부에서 주입되어, 같은 클래스가 여러 환경에서 재사용됩니다.
3. 유지보수성 증대:
    - 의존성 관리가 중앙화 되어 있으므로, 객체 간의 관계를 쉽게 파악하고 변경할 수 있습니다.
4. 테스트 용이성:
    - 의존성을 주입받고, 인터페이스에 의존하므로 만약 테스트를 할 시 모의 객체(Mock)을 주입하거나 직접 만든 테스트 객체를 주입하므로써 테스트를 수행할 수 있습니다. 

* 여기서 언급하는 중앙이란 Spring의 ApplicationContext을 의미한다.

## 2. Spring의 의존성 주입이란?

Spring 프레임워크는 의존성 주입(Dependency Injection, DI)을 핵심 기능으로 제공합니다.

이 챕터에서는 Spring의 DI에 대해서 살펴봅시다.

### 2.1 들어가기 전: @Autowired에 대하여

**`@Autowired`** 어노테이션은 Spring에서 가장 많이 사용되는 의존성 주입 방법 중 하나입니다.

이 어노테이션은 Spring 컨테이너가 필요한 Bean을 자동으로 주입하도록 지시합니다.

#### `@Autowired`의 주요 특징

- **자동 주입**: Spring이 타입을 기준으로 Bean을 찾아 주입합니다.
- **선택적 주입**: `required` 속성을 통해 주입 여부를 선택할 수 있습니다.

### 2.2 Setter 주입

Setter 주입은 의존성을 주입하기 위해 세터 메소드를 사용하는 방식입니다.


선택적으로 의존성 주입을 할 때 사용되며, 의존성을 변경해줍니다.
...
Field 주입은 의존성을 필드에 직접 주입하는 방식입니다.

필드에 직정 `@Autowired`를 사용합니다.
...
생성자 주입은 의존성을 생성자를 통해 주입하는 방식입니다.

주로 권장되는 방식으로, 불변성을 보장하고 테스트하기 용이합니다.
...
| 주입 방식    | 특징                                   | 
|----------|--------------------------------------| 
| Setter 주입 | 선택적 의존성이나 동적 의존성 변경에 유리             | 
| 필드 주입    | 간결하고 쉬움, 테스트나 객체 직접 생성에 취약           | 
| 생성자 주입   | 주로 추천되는 방식, 불변성 보장, 의존성이 명확함, 테스트 용이 |
...
**Bean**Factory는 스프링의 기본적인 Ioc 컨테이너로, 빈의 생성과 관리를 담당합니다.

ApplicationContext은 BeanFactory를 확장한 인터페이스로, 추가적인 부가기능을 제공합니다.

주요 차이점은 **로딩 방식**입니다.

BeanFactory: Lazy Loading 방식을 사용하여, 실제로 필요한 빈이 요청되기 전까지 빈을 로딩하지 않습니다.

**ApplicationContext**: Eager Loading 방식을 사용하여, 컨테이너가 초기화될 때 모든 빈을 미리 로딩합니다.

일반적으로는 ApplicationContext을 사용합니다. 

### 3.2 객체 찾기 방식
#### 3.2.1 byType
- 동작 방식 : 
  - `@Autowired`은 자동으로 byType 형식으로 주입합니다.  
  - 동일한 타입의 빈이 여러 개인 경우, `@Qualifier`을 통해 byName을 참고합니다.
...
타입 충돌 시 `@Primary` 혹은 `@Qualifier`로 해결 필요합니다.
주로 쓰이는 방법입니다.

#### 3.2.2 byName
- 동작 방식:
  - Bean 이름으로 객체를 검색합니다.
  - applicationContext.getBean("beanName") 형태로 호출합니다.
...
여러 개의 Bean 중에서 하나의 Bean을 선택한다면 예외가 발생하며 Spring은 선택을 내리지 못합니다.
 
`@Primary`를 붙인다면 Bean이 자동으로 선택됩니다.

이 빈이 기본 값이라고 선언하는 것과 같습니다.
...
스프링 애플리케이션에서 개발(dev), 운영(prod), 테스트(test) 등의 환경별로 다른 Bean을 쓰고 싶을 때, @Profile을 사용합니다.
...
### 4.3 @Qualifier
`@Autowired` 는 타입 기반으로 Bean을 등록하지만, 특정 이름으로 등록해야 할 때가 있습니다.

이떄 쓰이는 것이 `@Qualifier` 로 이름을 지정하여 사용됩니다.
...
순환 참조는 두 개 이상의 빈이 서로를 의존할 때 발생합니다.
예로 A는 B를 참조하고 B는 A가 참조할 때 무한 루프에 빠질 수 있습니다.
